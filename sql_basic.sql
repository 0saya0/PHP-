...
SELECT SUBSTRING(name , 1,1),name FROM `countries`
...
-- 問25
-- 全ての国と言語を表示してください。
SELECT name, language FROM `countries` LEFT OUTER JOIN `countrylanguages` ON `countries`.code = `countrylanguages`.country_code

-- 問26
-- 全ての国と言語と市区町村を表示してください。
SELECT `countries`.name, `cities`.district, `countrylanguages`.language 
FROM `countries` 
LEFT OUTER JOIN `cities`
ON `countries`.code = `cities`.country_code
LEFT OUTER JOIN `countrylanguages`
...
SELECT `celebrities`.name,
       (SELECT `countries`.name
             FROM `countries`
             WHERE `countries`.code = `celebrities`.`country_code`) AS countries_name
FROM `celebrities`
...
SELECT DATE_FORMAT(birth, '%Y' ),COUNT(DATE_FORMAT(birth, '%Y' )) FROM `celebrities` WHERE DATE_FORMAT(birth, '%Y' ) = '1980'
UNION
SELECT DATE_FORMAT(birth, '%Y' ),COUNT(DATE_FORMAT(birth, '%Y' )) FROM `celebrities` WHERE DATE_FORMAT(birth, '%Y' ) = '1981'
-- 日付変換(MySQLの場合)　→　DARE_FORMAT( カラム, '書式文字列')
